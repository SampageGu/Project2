...
static vector<char> allowed_chars;//对应ASCII码，记录可以输入的字符
//static NFANode NFANodes[MAX]; // 存储所有的 NFANode 对象
static void printNFAtable();
static int mDFA[MAX];          //用来存储结点合并 比如mDFA[2]=0,表示状态2可以和状态0合并，初始化为-1
static set<int>NFAFinalState;//用来记录NFA的终态
static bool isDFAFinal[MAX];   //用来记录每个DFA状态是否是终态，初始化为0
...
static int REpos[65536];//记录节点对应的执行语句
...
static void initClosure(NFANode* startNode) {
	// 起始状态一定是输入空转状态
	//先将起始节点放入状态集合中
	cout << startNode->id << "这里是首个id" << endl;
